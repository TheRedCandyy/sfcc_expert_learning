 ...
 * @returns {Object} an error object
 */
function placeOrder(req, order, fraudDetectionStatus) {
    var result = { error: false };
    ...
    try {
        ...
        // Code that goes over all the products that are in the order basket and the products that are in the wishlist
        // Remove from the wishlist all the products that are in the order basket
        ...
        var wishlist = productListHelper.getCurrentOrNewList(order.customer, { type: 10 });

        var products = order.productLineItems.toArray();

        products.forEach(function (product, index) {
            if(productListHelper.itemExists(wishlist, product.productID, config)) {
                productListHelper.removeItem(order.customer, product.productID, config);
            }
        });
